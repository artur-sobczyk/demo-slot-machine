...
This feature modernizes the existing slot machine application's AWS SAM template. The infrastructure includes an Amplify Hosting app (replacing S3 static hosting) with an optional custom domain parameter, a Cognito Identity Pool for unauthenticated frontend access, a Lambda function upgraded to nodejs22.x with AWS SDK v3, and a DynamoDB table with seed data populated via a custom resource. The project uses a monorepo structure with `backend/` for SAM infrastructure, `frontend/` for the browser app, and `local/` for development tooling. Local development uses Floci as a local AWS emulator, and the project includes a GitHub Actions deployment pipeline placeholder.

## Glossary

- **SAM_Template**: The AWS SAM template file (`backend/template.yaml`) that defines all infrastructure resources for the slot machine application using the `AWS::Serverless-2016-10-31` transform
...
- **Draw_Lambda**: The AWS Lambda function (in `backend/draw-lambda/`) that reads random slot positions from the DynamoDB table and returns slot machine results
...
- **Seed_Resource**: A CloudFormation custom resource defined in the SAM_Template that populates the Slot_Table with initial data during deployment, backed by a Lambda function in the `backend/seed-lambda/` directory
- **Custom_Domain_Parameter**: A SAM template parameter that accepts the custom domain name for Amplify hosting
- **SAM_Config**: The `backend/samconfig.toml` file that stores SAM CLI deployment configuration (stack name, region, capabilities, parameter overrides)

## Requirements

### Requirement 1: Project Structure

**User Story:** As a developer, I want the project organized with clear separation of backend, frontend, and local development concerns, so that the codebase is easy to navigate and maintain.

#### Acceptance Criteria

1. THE SAM_Template SHALL be defined in a `backend/template.yaml` file using the `AWS::Serverless-2016-10-31` transform
2. THE project SHALL include a `backend/samconfig.toml` file containing SAM CLI deployment configuration including stack name, region, and capabilities
3. THE project SHALL keep the Draw Lambda source code in a `backend/draw-lambda/` directory
4. THE project SHALL keep the Seed Lambda source code in a `backend/seed-lambda/` directory with shared seed data logic in a separate `seed-records.js` module
5. THE project SHALL keep the static frontend assets in a `frontend/static/` directory
6. THE project SHALL keep the frontend JavaScript source in a `frontend/src/` directory
7. THE project SHALL include a `.github/workflows/` directory at the project root for CI/CD pipeline definitions
8. THE project SHALL include a `local/` directory containing `docker-compose.yml`, `dev-server.js`, and `seed-local.js` for local development
...
2. THE Seed_Resource SHALL be implemented as a CloudFormation custom resource backed by a Lambda function located in the `backend/seed-lambda/` directory
...
7. THE seed data and write logic SHALL be defined in a shared `backend/seed-lambda/seed-records.js` module reused by both the Lambda handler and the local seed script
...
1. THE SAM_Template SHALL create the Draw_Lambda with the runtime set to nodejs22.x
2. THE SAM_Template SHALL deploy the Draw_Lambda with the CodeUri pointing to the `draw-lambda/` directory
3. THE SAM_Template SHALL configure the Draw_Lambda handler as "app.handler"
4. THE SAM_Template SHALL grant the Draw_Lambda read-only access to the Slot_Table using a SAM policy template (DynamoDBReadPolicy) scoped to the Slot_Table resource
5. THE SAM_Template SHALL set the Draw_Lambda memory to 128 MB
6. THE SAM_Template SHALL set the Draw_Lambda timeout to 10 seconds
7. THE SAM_Template SHALL pass the Slot_Table name to the Draw_Lambda as an environment variable named "TABLE_NAME"
8. IF the Draw_Lambda is invoked and the Slot_Table is unreachable, THEN the Draw_Lambda SHALL return an error response indicating that slot data could not be retrieved
...
2. THE frontend deployment script SHALL deploy the contents of the `frontend/static/` directory (including the built bundle) to the Amplify_App main branch as a single deployment artifact
...
2. THE SAM_Template SHALL output the Draw_Lambda function name with a descriptive output key
...
### Requirement 8: Local Development

**User Story:** As a developer, I want to run Floci (local AWS emulator) via docker-compose and invoke the Lambda function locally via a dev server, so that I can test and iterate on the application without deploying to AWS.

#### Acceptance Criteria

1. THE project SHALL include a `local/docker-compose.yml` file that runs Floci as a local AWS emulator
2. THE project SHALL include a `local/seed-local.js` script that populates the local Floci DynamoDB with the same 11 slot position records used in production, reusing the shared `seed-records.js` module
3. THE `local/docker-compose.yml` SHALL expose Floci on port 4566
4. THE `local/seed-local.js` script SHALL create the "SlotPositionTable" table with the correct schema before inserting records
5. THE project SHALL include a `local/dev-server.js` that serves the frontend and invokes the Lambda handler directly, with DynamoDB calls routed to Floci
6. THE project SHALL include documentation in a README file explaining how to start the local environment
7. THE Draw_Lambda source code SHALL support an optional `DYNAMODB_ENDPOINT` environment variable that overrides the default DynamoDB endpoint, enabling the function to connect to Floci when running locally
...
1. THE Draw_Lambda SHALL use the nodejs22.x runtime as specified in the SAM_Template function definition
2. THE Draw_Lambda source code SHALL use the AWS SDK v3 DynamoDB client (`@aws-sdk/client-dynamodb`) for all DynamoDB operations and SHALL NOT import or require the AWS SDK v2 (`aws-sdk`) package
3. THE SAM_Template SHALL define a "TABLE_NAME" environment variable on the Draw_Lambda whose value references the DynamoDB table resource name
4. THE Draw_Lambda source code SHALL read the DynamoDB table name exclusively from the "TABLE_NAME" environment variable (`process.env.TABLE_NAME`) instead of using a hardcoded table name
5. IF the "TABLE_NAME" environment variable is not set, THEN THE Draw_Lambda SHALL fail with an error indicating that the required TABLE_NAME configuration is missing
...
### Requirement 11: Frontend AWS SDK v3
...
1. THE frontend source code SHALL use `@aws-sdk/client-lambda` for Lambda invocations and `@aws-sdk/client-cognito-identity` for obtaining temporary credentials from the Cognito Identity Pool, and SHALL NOT use the AWS SDK v2 CDN bundle
...
3. THE frontend build SHALL use esbuild to produce a single bundled and minified JavaScript file (`frontend/static/app.bundle.js`) targeting ES2020 browsers
...
5. THE `frontend/deploy.ps1` script SHALL read CloudFormation stack outputs, replace placeholders in the frontend source, bundle with esbuild, and deploy the result to Amplify Hosting
